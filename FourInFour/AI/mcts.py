...
class _Node:
    ...
    __slots__ = ("board", "turn", "move", "parent", "children", "visits", "wins", "_untried", "winner", "winning_moves", "priority_moves", "if_priority")

    def __init__(self, board, turn, move=None, parent=None):
        self.board = board               # 棋盘状态（256 长度列表）
        self.turn = turn                 # 当前回合玩家
        self.move = move                 # 到达此节点的落子 idx
        self.parent = parent
        self.children = []
        self.visits = 0
        ...
        self.priority_moves = None  # 优先级落子列表
        self.if_priority = False  # 是否有优先级落子
    ...
    def get_priority_moves(self):
        ...
        """
        if self.priority_moves is None:
            current = self.turn
            next_p = (current % 3) + 1
            ...
            if self.winning_moves[current]:
                priority_moves.append(self.winning_moves[current][0])

            # 2. 下一个玩家有必胜点
            elif self.winning_moves[next_p]:
            ...
            elif len(self.winning_moves[last_p]) >= 2:
                for _ in range(2):
                    priority_moves.append(self.winning_moves[last_p][_])

            self.priority_moves = priority_moves
            if self.priority_moves:
                self.if_priority = True
        return self.priority_moves
...
class MCTSEngine:
    ...
    def _select(self, node):
        ...
        for _ in range(5):
            ...
            priority_moves = node.get_priority_moves()
            if priority_moves:
                # 有优先落子，直接返回该节点
                node = self._expand(node, priority_moves)
    ...
    def _expand(self, parent, untried):
        ...
        child = _Node(new_board, next_turn, move=idx, parent=parent)
