...
基于 **FastAPI + WebSocket** 的多人在线四维四子棋对弈平台，支持人类玩家与 AI 电脑玩家混合对弈，提供暂停与悔棋功能。
...
└── FourInFour/
    ...
    ├── AI/
    │   ├── __init__.py         # AI 包导出
    │   ├── mcts.py             # 蒙特卡洛树搜索 (MCTS) + UCB1 引擎（普通型）
    │   └── AB_mcts.py          # MCTS + UCB1 + Alphabet 剪枝引擎（Tian型）
    ├── GameCore/
    │   ├── __init__.py          # 统一导出所有对外接口
    │   ├── board.py             # 棋盘核心：4D↔2D 坐标映射、获胜线预计算
    │   ├── game.py              # 游戏房间状态管理（玩家注册、落子校验、胜负判定、暂停/悔棋）
    │   └── player.py            # 玩家类层次（Human / Random / MCTS-Normal / AB-Tian）
    ├── static/
...
| AI 算法 | 蒙特卡洛树搜索 (MCTS) + UCB1 + Alphabet 剪枝 |
...
- **Tian型（AB-MCTS）** — 在标准 MCTS 基础上引入 Alphabet 剪枝，通过必胜点分析减少无效搜索

### 算法详解

#### 1. 普通型 — MCTS + UCB1

完整实现了四阶段蒙特卡洛树搜索，每次落子前在后台运行搜索。
...
#### 2. Tian型 — MCTS + UCB1 + Alphabet 剪枝

在标准 MCTS 基础上引入 **Alphabet 剪枝**，通过必胜点分析减少搜索空间。
...
**AB 剪枝规则**（`point_to_try` 驱动）：

| 条件 | 行为 |
|------|------|
| 自己 ≥1 个必胜点 | 只走必胜点（其它分支不可能更好） |
| 下家恰好 1 个必胜点 | 必须堵（否则下家直接赢） |
| 上家 ≥2 个必胜点 | 必须堵一个（否则上家下回合必赢） |
| 以上均不满足 | 不剪枝，所有空位均可探索 |

**算法流程**
1. 从根节点出发，先检查必胜点
2. 通过必胜点，使用 Alphabet 剪枝树展开，直到不存在任何必胜点（无法剪枝）
3. 对所有根节点评分，随机模拟中遇到必胜点仍按必胜点优先落子
4. 每个节点维护三份分数，代表三个玩家的胜率
5. 所有叶节点至少探索一次后使用 UCB1 选择
6. 时间/迭代到上限后选访问次数最多的一级子节点
...
| 功能 | 说明 |
|------|------|
| **暂停（⏯）** | 玩家思考时按暂停 → 禁止落子；电脑思考时按暂停 → 取消思考不计步。继续后若轮到电脑则重新思考 |
| **悔棋（↩）** | 仅在玩家思考时或暂停时可用。撤销上一步棋并回退回合。若回退到电脑回合则自动暂停，按继续后电脑重新思考。胜利后仍可悔棋 |
...
| `reset` | 客户端→服务端 | 重置游戏 |
| `state` | 服务端→客户端 | 游戏状态同步（含 `paused` 字段） |
